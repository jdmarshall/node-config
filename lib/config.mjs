...
// Dependencies
import { RawConfig } from './raw.mjs';
import { Util, Load } from './util.mjs';
...
const Config = function() {
  ...
  for (const fnName in util) {
    if (typeof util[fnName] === 'function') {
      util[fnName] = util[fnName].bind(this);
    }
  }

  // Merge configurations into this
  _init(FIRST_LOAD);
  Util.extendDeep(this, FIRST_LOAD.config);
  util.attachProtoDeep(this);

  // Perform strictness checks and possibly throw an exception.
  util.runStrictnessChecks(this);
};
...
util.setModuleDefaults = function (moduleName, defaultProperties) {
  ...
  let existing = Util.getPath(this, path);

  if (existing === undefined) {
    Util.setPath(this, path, Util.cloneDeep(moduleConfig));
  } else {
  ...
  return util.attachProtoDeep(Util.getPath(this, path));
};
...
util.runStrictnessChecks = function (config) {
  ...
  FIRST_LOAD.options.nodeEnv.forEach(function(env) {
    ...
    const anyFilesMatchEnv = sourceFilenames.some((filename) => filename.match(env));
...
export { Config, Util, Load, config };
